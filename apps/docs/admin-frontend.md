...
| `round:end` | `store.endRound(id)` + invalidate `['session-messages', sessionId]` |
...
Дополнительно после `session:join` и после reconnect хук принудительно инвалидирует `['session-messages', sessionId]`, чтобы восстановить пропущенные ранние события (например, первое сообщение Директора, пришедшее до входа в room). Если пришёл `agent:message:chunk/end` для неизвестного `messageId` (пропущен `message:start`), хук тоже запускает backfill из REST.
...
Store поддерживает reconciliation через `mergeMessagesFromSnapshot` и `mergeRoundsFromSnapshot`: REST-снапшот дозаливает пропущенные сообщения и обновляет уже существующие (например, когда WS start/chunk был потерян, а финальное сообщение уже сохранено в БД).
...
// GET /api/sessions/:id/messages — первичная загрузка + reconciliation
// Query может рефетчиться после join/reconnect/round:end для backfill пропущенных WS-событий
```
